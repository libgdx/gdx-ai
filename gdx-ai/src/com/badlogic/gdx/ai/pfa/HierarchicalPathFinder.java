 ...
 * particular level. All future calls to the methods of the hierarchical graph then act as if the graph was just a simple,
 * non-hierarchical graph at that level. This way, the level path finder has no way of telling that it is working with a
 * hierarchical graph and it doesn't need to, meaning that you can use any path finder implementation for the level path finder.
 * 
...
public class HierarchicalPathFinder<N> implements PathFinder<N> {
	...
	public boolean searchNodePath (N startNode, N endNode, Heuristic<N> heuristic, GraphPath<N> outPath) {
		...
		int currentLevel = graph.getLevels() - 1;
		while (currentLevel >= 0) {
			// Find the start and end nodes at this level
			currentStartNode = graph.getNodeAtLevel(0, startNode, currentLevel);
			currentEndNode = graph.getNodeAtLevel(levelOfNodes, currentEndNode, currentLevel);
	...
	public boolean searchConnectionPath (N startNode, N endNode, Heuristic<N> heuristic, GraphPath<Connection<N>> outPath) {
		...
		while (currentLevel >= 0) {
			...
			// Now take the first move of this plan and use it for the next run through
			currentEndNode = outPath.get(0).getFromNode();
		}
